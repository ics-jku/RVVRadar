...
void png_filter_paeth_rvv_m2(unsigned int bpp, unsigned int rowbytes, uint8_t *row, uint8_t *prev_row)
{
	unsigned int vl = 0;
	...
	while (row < rp_end) {
		...
		asm volatile ("vsetvli		%0, %1, e8, m2" : "=r" (vl) : "r" (bpp));
	...
	while (row < rp_end) {
		...
		/* switch to widened */
		asm volatile ("vsetvli		zero, %0, e16, m4" : : "r" (bpp));
		...
		/* switch to narrow */
		asm volatile ("vsetvli		zero, %0, e8, m2" : : "r" (bpp));
